...
<div class="page-main">
	...
	<div class="content-main">
		...
		<form id="modify_form" action="modifySchedule.do" method="post">
		<input type="hidden" name="schedule_num" value="${schedule_num}">
			<ul>
				<li>
					<label for="team_category">경기종목</label>
					<input type="radio" name="team_category" id="team_category" value="0" <c:if test="${vo.team_category == 0}">checked</c:if>>축구
					<input type="radio" name="team_category" id="team_category" value="1" <c:if test="${vo.team_category == 1}">checked</c:if>>야구
					<input type="radio" name="team_category" id="team_category" value="2" <c:if test="${vo.team_category == 2}">checked</c:if>>배구
					<input type="radio" name="team_category" id="team_category" value="3" <c:if test="${vo.team_category == 3}">checked</c:if>>농구
				</li>
				<li>
					<label for="schedule_start">경기 시작일</label>
					<input type="text" name="schedule_start" id="schedule_start" value="${vo.schedule_start}">
				</li>
				<li>
					<label for="schedule_end">경기 종료일</label>
					<input type="text" name="schedule_end" id="schedule_end" value="${vo.schedule_end}">
				</li>
				<li>
					<label for="schedule_status">경기현황</label>
					<input type="radio" name="schedule_status" id="schedule_status" value="2" <c:if test="${vo.schedule_status == 2}">checked</c:if>>예정
					<input type="radio" name="schedule_status" id="schedule_status" value="3" <c:if test="${vo.schedule_status == 3}">checked</c:if>>취소
				</li>
				<li>
					<label for="schedule_team1">경기팀 1</label>
					<input type="number" name="schedule_team1" id="schedule_team1" value="${vo.schedule_team1}">
				</li>
				<li>
					<label for="schedule_team2">경기팀 2</label>
					<input type="number" name="schedule_team2" id="schedule_team2" value="${vo.schedule_team2}">
				</li>
			</ul>
			<div class="align-center">
				<input type="submit" value="수정">
				<input type="reset" value="취소">
				<input type="button" value="해당 경기 삭제" id="delete_btn">
				<script type="text/javascript">
					...
					delete_btn.onclick=function(){
						...
						if(choice){
							location.replace('deleteSchedule.do?schedule_num=${schedule_num}');
						}
					};
				</script>	
			</div>
