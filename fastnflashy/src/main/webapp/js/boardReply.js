$(function(){
	...
	function selectReplyList(pageNum){
		...
		$.ajax({
			...
			success:function(param){
				...
				if(pageNum == 1){
					//처음 호출시는 해당 id의 div 태그 내부 내용물을 제거
					$('#output').empty();
				}
	...
	$(document).on('click','.modify-btn',function(){
		...
		modifyUI += '<div id="re_second" class="align-right">';
		...
		//이전에 이미 수정하는 댓글이 있을 경우 수정버튼을 클릭하면 숨겨져 있는 div(class="sub-item")를 환원시키고 수정form을 초기화함(제거)
		initModifyForm();
		
		//지금 클릭해서 수정하고자 하는 데이터는 감추기
		//수정버튼을 감싸고 있는 div
		$(this).parent().hide(); //parent() -> div class="sub-item"을 말한다.
